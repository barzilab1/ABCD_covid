...
standardizedSolution(med_mod_mean_money, type = "std.all")
...
covid_wide %>% dplyr::filter(!is.na(money_cv_mean3to6) & !is.na(fam_wage_loss_cv_mean12) & !is.na(felt_sad_cv_raw_tot_bar_7)) %>% 
  dim() # 3498


# Residual of mediator
mod_mean_money_residM <- '
Residualized_money_cv ~ a*fam_wage_loss_cv_mean12
felt_sad_cv_raw_tot_bar_7 ~ c*fam_wage_loss_cv_mean12 + b*Residualized_money_cv
ind := a*b
total := c + a*b
'

med_mod_mean_money_residM <- sem(mod_mean_money_residM,
                                data = covid_wide,
                                se = "bootstrap",
                                bootstrap = 500)

standardizedSolution(med_mod_mean_money_residM, type = "std.all")

labels_mean_money_residM = list(
  fam_wage_loss_cv_mean12 = "Wage loss T1-2 \n (independent variable)",
  felt_sad_cv_raw_tot_bar_7 = "Depressive symptoms T7 \n (outcome)",
  Residualized_money_cv = "Worries about money T3-4-5-6 \n Regressed out other worries \n (mediator)"
)

plot_mean_money_residM <- lavaanPlot(
  model = med_mod_mean_money_residM,
  labels = labels_mean_money_residM,
  node_options = list(
    shape = "box",
    fontname = "Arial",
    color = "blue4"
...
save_png(plot_mean_money_residM, "../plots/plot_mean_money_residM.png" )

# Number of participants
covid_wide %>% dplyr::filter(!is.na(Residualized_money_cv) & !is.na(fam_wage_loss_cv_mean12) & !is.na(felt_sad_cv_raw_tot_bar_7)) %>% 
  dim() # 3492


# Residual of IV only
mod_mean_money_residIV <- '
money_cv_mean3to6 ~ a*Residualized_IV_cv
felt_sad_cv_raw_tot_bar_7 ~ c*Residualized_IV_cv + b*money_cv_mean3to6
ind := a*b
total := c + a*b
'

med_mod_mean_money_residIV <- sem(mod_mean_money_residIV,
                                data = covid_wide,
                                se = "bootstrap",
                                bootstrap = 500)

standardizedSolution(med_mod_mean_money_residIV, type = "std.all")

labels_mean_money_residIV = list(
  Residualized_IV_cv = "Wage loss T1-2 \n Regressed out covariates \n (independent variable)",
  felt_sad_cv_raw_tot_bar_7 = "Depressive symptoms T7 \n (outcome)",
  money_cv_mean3to6 = "Worries about money T3-4-5-6 \n (mediator)"
)

plot_mean_money_residIV <- lavaanPlot(
  model = med_mod_mean_money_residIV,
  labels = labels_mean_money_residIV,
  node_options = list(
    shape = "box",
    fontname = "Arial",
    color = "blue4"
...
save_png(plot_mean_money_residIV, "../plots/plot_mean_money_residIV.png" )
...
standardizedSolution(med_mod_mean_M1, type = "std.all")
...
standardizedSolution(med_mod_mean_M2, type = "std.all")
...
standardizedSolution(med_mod_mean_M3, type = "std.all")
...
standardizedSolution(med_model_1_mean_M1M2M3, type = "std.all")
...
standardizedSolution(med_model_1_mean_M1M2M3M4, type = "std.all")
...
covid_wide %>% dplyr::filter(
  !is.na(increased_conflict_cv_mean35) &
  !is.na(parent_monitor_tot_bar_cv_mean3to6) &
  !is.na(work_ability_cv_mean3to6) &
  !is.na(money_cv_mean3to6) &
    !is.na(fam_wage_loss_cv_mean12) &
    !is.na(felt_sad_cv_raw_tot_bar_7)
) %>%
  dim()
```
...
standardizedSolution(med_model_2_long_M1M2M3, type = "std.all")


labels_long_M1M2M3 = list(
  increased_conflict_cv_mean35 = "Family conflict \n (mediator 2)",
...
standardizedSolution(med_model_3_long_M1M2M3, type = "std.all")
...
standardizedSolution(med_model_2_long_M1M2M3M4, type = "std.all")
...
standardizedSolution(med_model_3_long_M1M2M3M4, type = "std.all")
...
# 4. RESIDUALS MEDIATION
## 4.1. Financial worries after regressing out other worries

```{r}
# Main model
model_1_mean_M1M2M3M4_regressout_M <- '
increased_conflict_cv_mean35 ~ fc1*fam_wage_loss_cv_mean12
parent_monitor_tot_bar_cv_mean3to6 ~ pm1*fam_wage_loss_cv_mean12
work_ability_cv_mean3to6 ~ pwc1*fam_wage_loss_cv_mean12
Residualized_money_cv ~ mw1*fam_wage_loss_cv_mean12
felt_sad_cv_raw_tot_bar_7 ~ mf*fam_wage_loss_cv_mean12 + fc2*increased_conflict_cv_mean35 + pm2*parent_monitor_tot_bar_cv_mean3to6 +
...
med_model_1_mean_M1M2M3M4_regressout_M <- sem(
  model_1_mean_M1M2M3M4_regressout_M,
  data = covid_wide,
  se = "bootstrap",
  bootstrap = 500
...
standardizedSolution(med_model_1_mean_M1M2M3M4_regressout_M, type = "std.all")


labels_1_mean_M1M2M3M4_regressout_M = list(
  increased_conflict_cv_mean35 = "Family conflict T3-5 \n (mediator 1)",
  fam_wage_loss_cv_mean12 = "Wage loss T1-2 \n (independent variable)",
  parent_monitor_tot_bar_cv_mean3to6 = "Parental monitoring T3-4-5-6 \n (mediator 2)",
  work_ability_cv_mean3to6 = "Parent-workplace conflict T3-4-5-6 \n (mediator 3)",
...
plot_mean_M1M2M3M4_regressout_M <- lavaanPlot(
  model = med_model_1_mean_M1M2M3M4_regressout_M,
  labels = labels_1_mean_M1M2M3M4_regressout_M,
  node_options = list(
    shape = "box",
    fontname = "Arial",
    color = "blue4"
...
save_png(plot_mean_M1M2M3M4_regressout_M, "../plots/mediator_parentsconflict/plot_mean_M1M2M3M4_regressout_M.png")
```

## 4.2. IV after regressing out covariates

```{r}
model_1_mean_M1M2M3M4_regressout_IV <- '
increased_conflict_cv_mean35 ~ fc1*Residualized_IV_cv
parent_monitor_tot_bar_cv_mean3to6 ~ pm1*Residualized_IV_cv
work_ability_cv_mean3to6 ~ pwc1*Residualized_IV_cv
money_cv_mean3to6 ~ mw1*Residualized_IV_cv
...
med_model_1_mean_M1M2M3M4_regressout_IV <- sem(
  model_1_mean_M1M2M3M4_regressout_IV,
  data = covid_wide,
  se = "bootstrap",
  bootstrap = 500
...
standardizedSolution(med_model_1_mean_M1M2M3M4_regressout_IV, type = "std.all")


labels_1_mean_M1M2M3M4_regressout_IV = list(
  increased_conflict_cv_mean35 = "Family conflict T3-5 \n (mediator 1)",
  Residualized_IV_cv = "Wage loss T1-2 \n Regressed out covariates \n (independent variable)",
  parent_monitor_tot_bar_cv_mean3to6 = "Parental monitoring T3-4-5-6 \n (mediator 2)",
  work_ability_cv_mean3to6 = "Parent-workplace conflict T3-4-5-6 \n (mediator 3)",
...
plot_mean_M1M2M3M4_regressout_IV <- lavaanPlot(
  model = med_model_1_mean_M1M2M3M4_regressout_IV,
  labels = labels_1_mean_M1M2M3M4_regressout_IV,
  node_options = list(
    shape = "box",
    fontname = "Arial",
    color = "blue4"
...
save_png(plot_mean_M1M2M3M4_regressout_IV, "../plots/mediator_parentsconflict/plot_mean_M1M2M3M4_regressout_IV.png")
```

## 4.3. Regress out both IV and M
...
standardizedSolution(med_model_1_mean_M1M2M3M4_regressout_M_IV, type = "std.all", output = "text")
